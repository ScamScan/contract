    ...
    Assertion.addProperty("reverted", function () {
        ...
        const onSuccess = (value) => {
            if (isTransactionResponse(value) || typeof value === "string") {
                ...
                return getTransactionReceipt(hash).then((receipt) => {
                    this.assert(receipt.status === 0, "Expected transaction to be reverted", "Expected transaction NOT to be reverted");
                });
            }
        ...
        const onError = (error) => {
            ...
            if (decodedReturnData.kind === "Empty" ||
                ...
                this.assert(true, null, `Expected transaction NOT to be reverted, but it reverted with reason '${decodedReturnData.reason}'`);
            }
            else if (decodedReturnData.kind === "Panic") {
                this.assert(true, null, `Expected transaction NOT to be reverted, but it reverted with panic code ${decodedReturnData.code.toHexString()} (${decodedReturnData.description})`);
            }
            else {
